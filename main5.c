...
int main(){
	...
	saveAt_2 = strcat(saveAt_2, "resultados/resuts5.cvs");
	...
	if(directory){
		...
		while( dir != NULL){
	        ...
	        Schedule *m =  build_cost_matrix(nsp, c5);
			m->cost_solution = cost_solution(m, c5, nsp);
				
			Schedule* rt = simulated_annealing(m,temp,finalTemp,it,reduction,vns, c5, nsp);	
