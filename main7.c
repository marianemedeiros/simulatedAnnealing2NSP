...
int main(){
	...
	saveAt_2 = strcat(saveAt_2, "resultados/resuts7.cvs");
	...
	if(directory){
		...
		while( dir != NULL){
	        ...
	        Schedule *m =  build_cost_matrix(nsp, c7);
			m->cost_solution = cost_solution(m, c7, nsp);
				
			Schedule* rt = simulated_annealing(m,temp,finalTemp,it,reduction,vns, c7, nsp);	
