...
int main(){
	...
	saveAt_2 = strcat(saveAt_2, "resultados/resuts3.cvs");
	...
	if(directory){
		...
		while( dir != NULL){
	        ...
	        Schedule *m =  build_cost_matrix(nsp, c3);
			m->cost_solution = cost_solution(m, c3, nsp);
				
			Schedule* rt = simulated_annealing(m,temp,finalTemp,it,reduction,vns, c3, nsp);	
