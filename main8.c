...
int main(){
	...
	saveAt_2 = strcat(saveAt_2, "resultados/resuts8.cvs");
	...
	if(directory){
		...
		while( dir != NULL){
	        ...
	        Schedule *m =  build_cost_matrix(nsp, c8);
			m->cost_solution = cost_solution(m, c8, nsp);
				
			Schedule* rt = simulated_annealing(m,temp,finalTemp,it,reduction,vns, c8, nsp);	
